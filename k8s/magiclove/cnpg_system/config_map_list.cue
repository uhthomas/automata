...
#ConfigMapList: items: [{
	metadata: name: "cnpg-default-monitoring"
	data: queries: yaml.Marshal({
		backends: {
			query: """
				SELECT sa.datname
				    , sa.usename
		...
		pg_replication: {
			query: "SELECT CASE WHEN ( NOT pg_catalog.pg_is_in_recovery() OR pg_catalog.pg_last_wal_receive_lsn() = pg_catalog.pg_last_wal_replay_lsn()) THEN 0 ELSE GREATEST (0, EXTRACT(EPOCH FROM (now() - pg_catalog.pg_last_xact_replay_timestamp()))) END AS lag, pg_catalog.pg_is_in_recovery() AS in_recovery, EXISTS (TABLE pg_stat_wal_receiver) AS is_wal_receiver_up, (SELECT count(*) FROM pg_catalog.pg_stat_replication) AS streaming_replicas"
			metrics: [{
				lag: {
					usage:       "GAUGE"
